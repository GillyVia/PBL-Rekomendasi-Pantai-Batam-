...
const THUMB_1 = "https://images.unsplash.com/photo-1662516705423-39f4b658eeb0?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxpbmRvbmVzaWElMjBiZWFjaCUyMGNyeXN0YWwlMjB3YXRlciUyMGFlcmlhbCUyMHZpZXd8ZW58MXx8fHwxNzc1ODM5OTQ2fDA&ixlib=rb-4.1.0&q=80&w=400";
const THUMB_2 = "https://images.unsplash.com/photo-1769451693880-c78dd975d951?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHx0cm9waWNhbCUyMGJlYWNoJTIwZ29sZGVuJTIwc3Vuc2V0JTIwc2lsaG91ZXR0ZSUyMHBhbG18ZW58MXx8fHwxNzc1ODM5OTQ5fDA&ixlib=rb-4.1.0&q=80&w=400";
...
export function HeroSection() {
  ...
  const filtered = SUGGESTIONS.filter((s) => !query || s.name.toLowerCase().includes(query.toLowerCase()));
        ...
        <div className="w-48 overflow-hidden rounded-2xl border border-white/20 bg-white/10 shadow-2xl shadow-blue-950/30 backdrop-blur-xl">
          ...
          <div className="px-3 py-2.5">
            <div className="flex items-center justify-between">
              <div><p className="text-xs font-bold text-white">Pantai Lagoi</p><p className="text-[10px] text-white/50">Bintan, Riau</p></div>
              <div className="flex items-center gap-0.5"><Star className="h-3 w-3 fill-amber-400 text-amber-400" /><span className="text-xs font-bold text-white">4.8</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute top-28 right-5 z-20 hidden md:block xl:right-16" style={{ animation: "floatBadge 4.5s ease-in-out infinite 0.3s" }}>
        <div className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3.5 shadow-2xl shadow-blue-950/30 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div><p className="text-xl leading-none font-black text-white">45+</p><p className="mt-0.5 text-[11px] font-medium text-blue-300">Pantai di Batam</p></div>
          </div>
        </div>
      </div>
        ...
        <div className="w-52 overflow-hidden rounded-2xl border border-white/20 bg-white/10 shadow-2xl shadow-blue-950/30 backdrop-blur-xl">
          ...
          <div className="px-3 py-2.5">
            <div className="flex items-center gap-2">
              <div className="flex h-5 w-5 items-center justify-center rounded-full bg-amber-400/20"><Waves className="h-2.5 w-2.5 text-amber-400" /></div>
              <div><p className="text-xs font-bold text-white">Spot Sunset #1</p><p className="text-[10px] text-amber-300">Pantai Melayu · 18:12</p></div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="overflow-visible rounded-3xl border border-white/60 bg-white/[0.97] shadow-[0_32px_80px_rgba(15,23,42,0.35)] backdrop-blur-2xl">
            <div className="p-3 sm:p-4">
                ...
                <div className="relative flex-1">
                  ...
                  {showDropdown && (
                    <div className="absolute top-full left-0 right-0 z-50 mt-2 overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-2xl shadow-blue-900/15">
                      <div className="px-4 pt-3 pb-1.5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Saran Destinasi</p></div>
                      {filtered.map((s) => (
                        <button key={s.name} onMouseDown={() => { setQuery(s.name); setFocused(false); }} className="group/sug flex w-full items-center gap-3 px-4 py-2.5 transition-colors hover:bg-blue-50">
                          <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-xl bg-blue-50 transition-colors group-hover/sug:bg-blue-100"><MapPin className="h-3.5 w-3.5 text-blue-500" /></div>
                      ...
                      <div className="border-t border-slate-100 bg-slate-50 px-4 py-2.5">
                        <p className="text-xs text-slate-400">Tekan <kbd className="rounded bg-slate-200 px-1.5 py-0.5 font-mono text-[10px] text-slate-600">Enter</kbd> untuk mencari semua hasil</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
                ...
                {QUICK_TAGS.map((tag) => (
                  <button key={tag.value} onClick={() => setQuery(tag.value)} className="rounded-full border border-blue-100 bg-blue-50 px-3 py-1.5 text-[11px] font-semibold text-blue-700 transition-all hover:border-blue-200 hover:bg-blue-100 hover:text-blue-800">{tag.label}</button>
                ))}
              </div>
            </div>
          </div>
